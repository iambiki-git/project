...
<?php
  ...
  ?>
    <h1 class="text-center mb-5 font-weight-bold mt-5">Billing Details</h1>
    <div class="row">
      <div class="col-md-6 m-auto">
        <form action="#" method="POST" id="myForm">
          ...
          <div class="form-group">
            <label for="ship_address">Delivery Address?<span class="text-danger">*</span></label><br>
            <input type="radio" id="billing_address" name="delivery_address" value="billing_address" required> billing address <br>
            <input type="radio" id="different_address" name="delivery_address" value="different_address" required> different address
          </div>
          
          <button type="submit" class="btn btn-primary" name="submit">Submit</button>
          <a href="shipping.php?order_id=<?php echo $order_id; ?>" class="btn btn-primary float-right custom-margin" id="next" style="display:none;">Next</a>
          <a href="profile.php" class="btn btn-secondary float-right">Previous</a>
        </form>
      </div>
    </div>
  </div>
    ...
    <?php
      ...
      if(isset($_POST['submit'])){
        ...
        $billing_query = "INSERT INTO billing_details (firstname, lastname, company_name, state,	city,	
        street_address, phone_no,	invoice_number, delivery_address) VALUES ('$firstname', '$lastname', '$company_name',
        '$state', '$city', '$street_address', '$phone', $invoice_number, '$delivery_address')";
        $result = mysqli_query($conn, $billing_query);  
        if($result){
          echo "<script>alert('Billing Details Accepted Successfully. Please click next to go further process')</script>";
          echo "<script>document.getElementById('next').style.display = 'block';
          </script>";
        }else {
          echo "<script>alert('Failed to insert billing details!')</script>";
        }
    ...
    <!-- this js code is for showing next button after submitting the billing details -->
<!-- <script>
